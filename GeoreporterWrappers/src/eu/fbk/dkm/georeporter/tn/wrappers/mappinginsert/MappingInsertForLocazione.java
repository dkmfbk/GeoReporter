		...
		for (int j = 0; j < listLocazione.size(); j++) {
			...
			for (int i = 0; i < data.getAttributi().size(); i++) {
				...
				if ((listLocazione.get(j).getValori().get(parts[1]) != null)
					...
					if (parts[1].contains("data")) {
						tmp.setValore(ControlloValore.controlloData(listLocazione.get(j).getValori().get(parts[1])));
					} else {
						tmp.setValore(ControlloValore.controlloVIR(listLocazione.get(j).getValori().get(parts[1])));
					}
			...
			for (int i = 0; i < data2.getAttributi().size(); i++) {
				...
				if ((listLocazione.get(j).getValori().get(parts2[1]) != null)
					...
					tmp2.setValore(ControlloValore.controlloVIR(listLocazione.get(j).getValori().get(parts2[1])));
