...
import eu.fbk.dkm.georeporter.ita.wrappers.WrapperFabITA;
...
public class MappingInsertFabIndirizzoITA {

	public  List<Indirizzo> listIndirizzi;
	public WrapperFabITA wf = new WrapperFabITA();
	public  String targetURL;
	
	public MappingInsertFabIndirizzoITA( String targetURL_) {
	...
	public  void associazioneMappingNomeVal(MappingTabella data) {
		...
		for (int j = 0; j < listIndirizzi.size(); j++) {
			...
			for (int i = 0; i < data.getAttributi().size(); i++) {
				...
				if (parts[1].equals(nameMappingsIndirizzoHM.get("codiceAmministrativo"))) {
					tmp.setValore(listIndirizzi.get(j).getListaValoriChiave().get(i).get(nameMappingsIndirizzoHM.get("codiceAmministrativo")));
					//tmp.setMapping(data.getAttributi().get(i).getMapping());
